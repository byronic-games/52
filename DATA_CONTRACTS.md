...
- Daily share suit totals are local-only metadata stored in `hl_prototype_daily_attempts_local` as `suitCounts`. They are not currently part of the Supabase contract; remote/historical attempts may share without suit rows. Visible share copy intentionally does not name Normal/Hard, but the generated URL still carries the active variant.
